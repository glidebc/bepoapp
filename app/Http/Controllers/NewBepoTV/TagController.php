...
class TagController extends Controller {
    public function getAutocomplete(Request $request){
        ...
        if($request->has('q')&&strlen($request->input('q'))>0){
            foreach( TagModel::where('name','like','%'.$request->input('q').'%')->limit(16)->get() as $m){
                $data[]=array(
                    'id'=>$m->_id,
                    'name'=>$m->name
                );
	...
	public function getIndex() {
		$filter=DataFilter::source(new TagModel());
        ...
        $grid->add('total','影片數量',true)->style('width:20%');
 $grid->add('created_at','建立時間',true)->style('width:20%');
		...
		$grid->orderBy('total','desc','name','asc');
	...
	public function anyEdit() {
		$edit=DataEdit::source(new TagModel());
