...
class VideosController extends Controller {
    private function getVideoId(){
      ...
      return 'vcbou2'.date('Ymd').$password;
    }

	public function getIndex() {
	    $source=Videos::join('video_class','video.vc_id','=','video_class.vc_id')
            ->where('video.p_id','=','pg20160202175139')
            ->where('video_class.active','1')
            ->select(DB::raw('video.*'));
		...
		$grid -> add('v_title', '標題', true) -> style('width:20%')-> cell(function($value, $row) {
            $href='<a target="_blank" href="http://showbar.ctitv.com.tw/v/video/%s.htm">%s</a>';
            return sprintf($href,$row->v_id,$value);
        });


        $grid->add('source_code_youtube','縮圖',false)-> cell(function($value, $row) {
            $t='<img style="width:90%%;" src="http://img.youtube.com/vi/%s/0.jpg">';
		...
		//copyToClipboard
		//http://showbar.ctitv.com.tw/v/pure_clickme/vcQDB20150714115404573474.htm?wmode=opaque
		...
		$grid -> add('creat_datetime', '建立時間', true) -> style('width:12%');
			$grid -> add('copyTo', '嵌入碼', false) -> cell(function($value, $row) {
                ...
                //http://showtv.ctitv.com.tw/v/clickme/20170123/26909
				$iframe=sprintf(
					'<iframe src="http://showbar.ctitv.com.tw/v/clickme/%s.htm?wmode=opaque"  class="ctitv-video-0" width="300" height="169" scrolling="no" allowfullscreen frameborder="0"></iframe>'
					,$row->v_id);
				$extend=sprintf(
		...
		$grid -> edit('showbar_videos/edit', '操作', 'show|modify') -> style('width:12%');
		$grid -> orderBy('video.sort', 'asc');
		$grid -> orderBy('video.creat_datetime', 'desc');
		$grid -> link('showbar_videos/edit', "新增", "TR");
	...
	public function anyEdit() {
		...
		$edit -> link("showbar_videos", "回列表", "TR") -> back();
