...
class NewsResource extends Controller {
     ...
     */
    private function get_news_by_category($kind,$limit=0) {
    ...
    public function news_test() {
        ...
        if(Input::get('kind')>0) {
            ...
            if(Cache::has($key)) {
                ...
                $datalist=$this->get_news_by_category(Input::get('kind'));
    ...
    public function ad(Request $request) {
        ...
        if(strlen($platform_type)) {
            ...
            //Get by random
            $data=Ad::where('status','=','1')->where('begin_at','<',$now)->where('end_at','>',$now)
                ->where('platform_type','like','%'.$platform_type.'%')->where('is_default','=','0')->select('*')->orderBy(DB::raw('RAND()'))->first();
            if(count($data)>0) {
                //$logFile='ad.log';
                //Log::useDailyFiles(storage_path().'/logs/ad/'.$logFile);
                //Log::info(json_encode($_SERVER,JSON_UNESCAPED_UNICODE));
                ...
                //get default
                $data=Ad::where('status','=','1')->where('begin_at','<',$now)->where('end_at','>',$now)
                    ->where('platform_type','like','%'.$platform_type.'%')->where('is_default','=','1')->select('*')->orderBy(DB::raw('RAND()'))->first();
                if(count($data)>0) {
                    $embed=$data->embed;
                    $data=$data->toArray();
                    $data['embed']=$embed;
    ...
    public function channel24hours() {
        $datalist='https://www.youtube.com/watch?v=hgIfZz8STLk';
        $datalist='https://www.youtube.com/channel/UCpu3bemTQwAU8PqM4kJdoEQ/videos';
