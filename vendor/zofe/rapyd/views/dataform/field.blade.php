...
@else
    <div class="form-group clearfix{!!$field->has_error!!}" id="fg_{!! $field->name !!}" >
        ...
        @if ($field->has_label)
            <label for="{!! $field->name !!}" class="col-sm-2 control-label{!! $field->req !!}">{!! $field->label !!}@if($field->required)<i class="fa fa-asterisk" style="font-size:0.5em;text-shadow: 1px 1px 1px #ccc;color:#ff0000;" aria-hidden="true"></i>@endif</label>
