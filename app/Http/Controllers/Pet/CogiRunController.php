...
class CogiRunController extends Controller {
    public $opts=array(
        ''=>'狀態',
        '0'=>'等待審核',
        '1'=>'通過',
    ...
    public function getIndex() {
        ...
        $grid->add('review','操作',false)->cell(function($value,$row) {
            ...
            $return.='<a class="" title="明細資料" href="cogi_run_review/edit?show='.$row['id'].'"><span class="btn-sm btn-info btn">明細</span></a>';
            $return.='&nbsp;<a class="" title="明細資料" href="cogi_run_review/allow?show='.$row['id'].'"><span class="btn-sm btn-success btn">通過</span></a>';
            $return.='&nbsp;<a class="" title="明細資料" href="cogi_run_review/deny?show='.$row['id'].'"><button class="btn-sm btn-danger btn">不通過</button></a>';
            return $return;
        });
    ...
    public function anyEdit() {
        ...
        $edit->link("cogi_run_review","回列表","TR")->back();
        ...
        $edit->add('phone','手機','number')->rule('required');
        $edit->add('petname','寵物名稱','text')->rule('required');
        $edit->add('petage','寵物年齡','text');
        $edit->add('description','描述','textarea');
        $edit->add('pic_path','Photo','App\Fields\ImageView')->preview(320,180);
