...
class VideosController extends Controller {
	public function getIndex(Request $request) {
		$m=VideoModel::orderBy('sort','asc');
		$filter=DataFilter::source($m);
		$filter->add('title','標題','text');
		$opts=\App\Model\NewBepoTV\CategoryModel::pluck('_id','name')->toArray();
		$temp=array();
		foreach($opts as $k=>$v) {
			$temp[$v]=$k;
		}
		...
		$filter->add('program','節目','select')->options(array(''=>'節目')+OptHelper::getProgram())->scope(function($query,$value) {
			if(!is_null($value)) {
				return $query->whereIn('program._id',array($value));
			}
			else {
				return $query;
			}
       ...
       $programId=$request->input('program');
        ...
        $filter->add('video_class','分類','select')->attr('size',20)->options($VCOpts)->scope(function($query,$value) {
            if(!is_null($value)) {
                return $query->whereIn('video_class._id',array($value));
            }
            else {
                return $query;
            }
...
/*
 $grid -> add('copyTo1', '主嵌入碼', false) -> cell(function($value, $row) {
                $button=sprintf(
                    '<a href="javascript:copyToClipboard(document.getElementById(\'copy1-id-%s\'));" >複製</a>',
                    $row->_id);
                $iframe=sprintf(
                    '<div class="video-player-wrapper-0"><iframe src="%s/embed/%s?autoplay=1" width="300" height="169" scrolling="no" allowfullscreen frameborder="0"></iframe></div>'
                    ,env('BEPO_URL')
                    ,$row->_id);
                $extend=sprintf(
                    '<input readonly=true type="text" style="width:80%%;" id="copy1-id-%s" value=\'%s\'>',
                    $row->_id,$iframe);
  ...
  */          
        ...
        $grid -> add('copyTo', '嵌入碼', false) -> cell(function($value, $row) {
                $button=sprintf(
                    '<a href="javascript:copyToClipboard(document.getElementById(\'copy-id-%s\'));" >複製</a>',
                    $row->_id);
                $iframe=sprintf(
                    '<div class="video-player-wrapper-0"><iframe src="%s/embed/%s" width="300" height="169" scrolling="no" allowfullscreen frameborder="0"></iframe></div>'
                    ,env('BEPO_URL')
                    ,$row->_id);
                $extend=sprintf(
                    '<input readonly=true type="text" style="width:80%%;" id="copy-id-%s" value=\'%s\'>',
                    $row->_id,$iframe);
                return $button.$extend;
            })->style('width:10%') ;

		$grid->add('created_at','建立時間',true)->style('width:10%');
   ...
   "hits_plus":null
    ...
    public function anyImport(Request $request) {
        ...
        foreach($files as $file){
            ...
            if(count($dl)==0)continue;
               foreach($dl as $programName => $d1){
                    $pModel=ProgramModel::where('name','=',$programName)->first();
                    foreach($d1 as $videoClassName=>$d2){
                        //clear video_class
                        VideoClassModel::where('name','=',$videoClassName)
                            ->where('program','=',$programName)
                            ->delete();
                        //clear all videos
                      VideoModel::whereIn('video_class.name',array($videoClassName))->
                        whereIn('program.name',array($programName))->delete();
	...
	public function anyEdit(Request $request) {
       ...
       if($model==null)$model=new VideoModel();
      ...
      //   $edit->add('small_pic',sprintf('列表專用圖(%sx%s)',env('IMAGE_SMALL_WIDTH'),env('IMAGE_SMALL_HEIGHT')),'text');
        $edit->add('small_pic',sprintf('列表專用圖(%sx%s)',env('IMAGE_SMALL_WIDTH'),env('IMAGE_SMALL_HEIGHT')),'image')->rule('required')->rule('mimes:jpeg,jpg,png,gif')
         ->preview(round(env('IMAGE_SMALL_WIDTH')/2),round(env('IMAGE_SMALL_HEIGHT')/2))->resize(env('IMAGE_SMALL_WIDTH'),env('IMAGE_SMALL_HEIGHT'))
          ->move('newbepotv/video/small/',$filename.'.'.$extension);
